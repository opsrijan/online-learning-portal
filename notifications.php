...
?>
            ...
            <hr><br>
            <?php
                $sql = "SELECT * FROM `announcement` WHERE CourseID IN (
                            SELECT CourseID FROM registration WHERE StudentID = ".$_SESSION['logger']."
                        ) ";
                $result = mysqli_query($con, $sql);
                while ($row=mysqli_fetch_assoc($result)){
                    $subsql = "SELECT * FROM `course` WHERE CourseID =".$row['CourseID'];
                    $result = mysqli_query($con, $subsql);
                    $rowx=mysqli_fetch_assoc($result);
                    echo "Course: ".$rowx['Title'];
                    echo "<br>";
                    $sql1 = "SELECT * FROM `instructor` WHERE InstructorID = ".$rowx['InstructorID'];
                    $res1 = mysqli_query($con, $sql1);
                    $x = mysqli_fetch_assoc($res1);
                    echo "Instructor Name: ".$x['Name'];
                    echo "<br>";
                    echo $row['Date'];
                    echo "<br><b>". $row['Message'];
                    echo "</b><hr>";
                }
            ?>
            <hr>
